...
export const Ques = ()=> {
  ...
  return (
    <div class="h-full w-full flex flex-col justify-center">
        {loading ?<div class="text-center text-white text-2xl"> Question Loading... </div>: 
        <div>
          ...
          {triviaQuestion.map((triviaData, index) =>
            <div class="text-2xl text-left text-white pl-10" key={index}>
              <div>
              ...
              <div>
                {
                  allPossibleAnswers.map((answer, index) =>
                    <div key={index}>
                      <ul>
                        <li><button class = "hover:text-3xl" key={index} onClick={() => verifyAnswer(answer)}>- {removeCharacters(answer)} </button></li>  
                      </ul>
                    </div>
                  )
                }
